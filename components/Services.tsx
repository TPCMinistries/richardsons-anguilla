...
import { Waves, Trophy, Baby, Check, ArrowRight } from "lucide-react";
import { SERVICES, SITE } from "@/lib/constants";
import { fadeUp, staggerContainer } from "@/lib/animations";

const ICONS: Record<string, React.ReactNode> = {
  swimming: <Waves className="h-10 w-10 text-ocean" />,
  football: <Trophy className="h-10 w-10 text-ocean" />,
  childcare: <Baby className="h-10 w-10 text-ocean" />,
};

const GRADIENTS: Record<string, string> = {
  swimming: "from-ocean/10 to-ocean/5",
  football: "from-orange/10 to-orange/5",
  childcare: "from-navy/10 to-navy/5",
};
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        >
          ...
          <motion.p variants={fadeUp} className="mx-auto max-w-2xl text-lg text-navy/50">
            Every service is backed by professional certification, years of experience, and a
        ...
        >
          {SERVICES.map((service) => (
            ...
            >
              {/* Icon header */}
              <div className={`relative flex h-52 items-center justify-center bg-gradient-to-br ${GRADIENTS[service.slug]}`}>
              ...
              <div className="flex flex-1 flex-col p-7">
                ...
                {/* CTA */}
                <a
                  href={`https://wa.me/${SITE.whatsapp}?text=Hi!%20I'm%20interested%20in%20${encodeURIComponent(service.title)}%20for%20my%20child.`}
                  target="_blank"
